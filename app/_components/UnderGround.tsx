...
const UnderGround = () => {
  ...
  const underGroundRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleScroll = (event: WheelEvent) => {
      ...
      const underGround = underGroundRef.current;

      if (scrollContainer && underGround) {
        const rect = underGround.getBoundingClientRect();
        const isUnderGroundInView =
          rect.top <= 0 && rect.bottom >= window.innerHeight;

        if (isUnderGroundInView) {
  ...
  return (
    <div ref={underGroundRef} className="relative w-full h-screen z-20">
      <div
