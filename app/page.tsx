import React from "react";
import Top from "./_components/Top";
import Intoroduction from "./_components/Intoroduction";

const undergroundGameInfo = [
  {
    id: 1,
    image: "/game.jpg",
    alt: "Game 1",
    description: "Game 1 description goes here.",
    url: "/example",
  },
  {
    id: 2,
    image: "/game.jpg",
    alt: "Game 2",
    description: "Game 2 description goes here.",
    url: "/example",
  },
  {
    id: 3,
    image: "/game.jpg",
    alt: "Game 3",
    description: "Game 3 description goes here.",
    url: "/example",
  },
  {
    id: 4,
    image: "/game.jpg",
    alt: "Game 4",
    description: "Game 4 description goes here.",
    url: "/example",
  },
  {
    id: 5,
    image: "/game.jpg",
    alt: "Game 5",
    description: "Game 5 description goes here.",
    url: "/example",
  },
  {
    id: 6,
    image: "/game.jpg",
    alt: "Game 6",
    description: "Game 6 description goes here.",
    url: "/example",
  },
];

const underGroundComments = [
  "Discover the adventure in Game 1!",
  "Game 2 awaits you with new challenges!",
  "Explore the universe of Game 3!",
  "Game 4 is just around the corner!",
  "Prepare for an epic journey in Game 5!",
  "Game 6: The final frontier!",
];

const page = () => {
  return (
    <div>
      <Top />
      <Intoroduction
        label="ラインナップ"
        labelColor="white"
        bgImage="/underground.jpg"
        gameInfo={undergroundGameInfo}
        commentList={underGroundComments}
        humanImage="/human.png"
        streamingImage="/human.png"
        buttonBgColor="white"
        buttonTextColor="black"
      />
    </div>
  );
};

export default page;
